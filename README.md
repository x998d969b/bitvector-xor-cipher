...
| Key Generation | Random key with length ≥ plaintext length. In this implementation, for simplicity, we generate a key that is plaintext length + 32 bytes (256 bits extra). The extra bytes are unused. Classic OTP requires: key length ≥ message length (not necessarily longer).|
